...
const ImporterSidebar: React.FC<ImporterSidebarProps> = ({ 
  activeService, setActiveService, onLogout, userProfile, enabledServices, 
  onUpgradeClick, isOpen, onClose, isCollapsed, onToggleCollapse 
}) => {
  const isFreeImporter = userProfile?.plan === 'FREE';
  ...
  const items = [
    ...
    { type: ServiceType.SOUQ_STORE, icon: 'store', label: 'Stores', pro: false },
  ...
  const getPlanColorClass = () => {
    return userProfile?.plan !== 'FREE' ? 'text-[#0df20d]' : 'text-white/40';
  };
  ...
  return (
    <>
      ...
      `}>
        <div className={`p-8 flex items-center gap-4 shrink-0 ${isCollapsed ? 'justify-center' : ''}`} onClick={() => setActiveService(ServiceType.MARKET_LOOK)}>
          <div className="size-12 bg-[#0df20d] rounded-2xl flex items-center justify-center text-black shadow-[0_0_20px_rgba(13,242,13,0.6)] rotate-3 cursor-pointer">
            <span className="material-symbols-outlined font-black text-3xl">hub</span>
          </div>
          {!isCollapsed && (
            <div className="cursor-pointer">
              <h1 className="text-2xl font-black tracking-tighter italic leading-none">SOUQ<span className="text-[#0df20d]">HUP</span></h1>
              <p className="text-[9px] text-[#0df20d]/60 uppercase font-black tracking-[0.2em] mt-1 italic">Trade Distribution</p>
            </div>
          )}
        ...
        <div className={`px-6 mb-10 ${isCollapsed ? 'flex justify-center' : ''}`}>
          <div className={`flex flex-col items-center text-center group cursor-pointer transition-all ${isCollapsed ? 'w-12' : ''}`} onClick={() => setActiveService(ServiceType.USER_PROFILE)}>
            <div className="relative mb-4">
              <div className="absolute -inset-1 rounded-full blur-md opacity-40 group-hover:opacity-100 transition-opacity bg-[#0df20d]"></div>
              <div className="relative size-24 rounded-full border-4 border-[#0df20d]/20 p-1 bg-[#08090b] overflow-hidden">
                <img src={userProfile.avatar} className="size-full rounded-full object-cover" alt="User" />
              </div>
              <div className="absolute bottom-1 right-1 size-6 bg-[#0df20d] rounded-full border-4 border-[#08090b] flex items-center justify-center shadow-[0_0_10px_rgba(13,242,13,0.4)]">
                <span className="material-symbols-outlined text-[14px] text-black font-black">verified</span>
              </div>
            </div>
            {!isCollapsed && (
              <div className="space-y-1">
                <p className="text-lg font-black tracking-tight text-white group-hover:text-[#0df20d] transition-colors leading-none">{userProfile.name}</p>
        ...
        <nav className={`flex-1 px-4 space-y-2 mt-4 ${isCollapsed ? 'items-center flex flex-col' : ''}`}>
          {items.map((item) => {
            ...
            const isProLocked = isFreeImporter && 
                  ...
                  <div className="flex-1 text-left min-w-0">
                    ...
                    {isProLocked && (
                      <div className="flex items-center gap-1">
                        <span className="material-symbols-outlined text-[10px] text-[#0df20d]">lock</span>
                        <p className="text-[7px] font-black uppercase tracking-widest text-[#0df20d] italic">Pro Locked</p>
                      </div>
                    )}
                  </div>
                )}
